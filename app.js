    ...
    try {
        ...
        if (result.success && result.data.length > 0) {
            ...
            result.data.forEach(agent => {
                const iconStyle = `background-color: ${agent.color || '#667eea'}`;
                const categoryBadge = `<span class="badge bg-primary">${escapeHtml(agent.category)}</span>`;

                const card = `
                    <div class="col-md-6 col-lg-4 mb-4">
                        <div class="card agent-card h-100" href='${agent.url || agent.id}' onclick="window.location.href='${agent.url || agent.id}'">
                            <div class="card-body">
